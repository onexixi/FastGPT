...
const emailMap: { [key: string]: any } = {
  [UserAuthTypeEnum.register]: {
    subject: '注册 MI TALK 账号',
    html: (code: string) => `<div>您正在注册 MI TALK 账号，验证码为：${code}</div>`
  },
  [UserAuthTypeEnum.findPassword]: {
    subject: '修改 MI TALK 密码',
    html: (code: string) => `<div>您正在修改 MI TALK 账号密码，验证码为：${code}</div>`
  }
};
